...
- 📄 **原始格式导出** - 导出与原始账单格式一致的 CSV 文件，保留所有原始列（订单号、商家订单号等），仅替换分类列，方便导入第三方记账软件
...
| `/api/export/original/<source>` | GET | 导出原始格式 CSV（source: alipay/wechat/bank） |
| `/api/clear` | POST | 清空所有数据 |

### 原始格式导出说明

原始格式导出功能可以导出与原始账单格式一致的 CSV 文件：

- **保留所有原始列**：订单号、商家订单号、付款方式、交易状态等
- **替换分类列**：将原始的"交易分类"列替换为系统自动分类的结果
- **按来源分开发**：支持支付宝、微信、银行三种格式分别导出
- **便于第三方导入**：导出的文件可直接导入第三方记账软件
